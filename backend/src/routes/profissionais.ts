...
export async function profissionaisRoutes(app: FastifyInstance) {
  ...
  app.get("/public/profissionais", async () => {
  ...
  app.get("/profissionais", { preHandler: adminMiddleware }, async () => {
    return prisma.profissional.findMany({
      orderBy: {
        createdAt: "desc",
      },
  ...
  app.delete(
    ...
    async (request, reply) => {
      ...
      if (profissional.atendimentos.length > 0) {
        return reply.status(400).send({
          message:
            "Não é possível excluir profissional com atendimentos vinculados",
        });
      }
